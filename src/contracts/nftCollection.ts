...
export default class NFTCollection implements Contract {
  static createForDeploy(
    ...
    console.log("Code cell:", initCode);
    console.log("Data cell:", initData);
  ...
  async getCollectionData(provider: ContractProvider) {
    ...
    let nextItemIndex = stack.readNumber().toString();
    let content = parseOffchainUriCell(stack.readCell());
    let owner = stack.readAddress().toString({ testOnly: true });
    return { nextItemIndex, content, owner };
  }

  async getRoyaltyParams(provider: ContractProvider) {
    const { stack } = await provider.get("royalty_params", []);
    return stack.readNumber(), stack.readNumber;
  }
}
